...
test: clean
	...
	cd ${TESTSITE_DIR}; ../../baker header --file index.text updated-time --set 2019-11-13T23:59:59+01:00
	cd ${TESTSITE_DIR}; ../../baker blog "This is a test post"
	cd ${TESTSITE_DIR}; echo "This is a test post" >> blog/2019/10/07/this-is-a-test-post.text
	cd ${TESTSITE_DIR}; ../../baker header --file blog/2019/10/07/this-is-a-test-post.text updated-time --set 2019-11-07T23:59:59+01:00
	cd ${TESTSITE_DIR}; echo "This is a test micro post" | ../../baker micro -
	cd ${TESTSITE_DIR}; ../../baker header --file micro/2019/10/07/205712.text updated-time --set 2019-11-07T23:59:59+01:00
	mkdir ${TESTSITE_DIR}/notebook; cp test/notebook.text ${TESTSITE_DIR}/notebook/
	cd ${TESTSITE_DIR}; ../../baker header --file notebook/notebook.text updated-time --set 2019-11-13T23:59:59+01:00
